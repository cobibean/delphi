...
export default function DirectListingPage() {
  ...
  const getERC721Contract = (address: string, signer?: ethers.Signer) => {
    const provider = signer || new ethers.providers.JsonRpcProvider(metisChain.rpc);
    return new ethers.Contract(address, ERC721ABI, provider);
  };
  
  // Helper function to create a marketplace contract
  const getMarketplaceContract = (signer?: ethers.Signer) => {
    const provider = signer || new ethers.providers.JsonRpcProvider(metisChain.rpc);
    return new ethers.Contract(CONTRACT_ADDRESS, MarketplaceABI, provider);
  };
