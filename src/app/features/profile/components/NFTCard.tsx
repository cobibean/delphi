...
import { NFTItem } from "../hooks/useUserNFTs";
import "./styles.css";
...
export default function ProfileNFTCard({ nft }: ProfileNFTCardProps) {
  ...
  useEffect(() => {
    const fetchNFTMetadata = async () => {
      try {
        if (nft.contractAddress && nft.id) {
          ...
          const metadata = await getNFTMetadata(nft.contractAddress, nft.id);
    ...
    fetchNFTMetadata();
  }, [nft.contractAddress, nft.id, currentGatewayIndex]);
  ...
  return (
    <div className="bg-night/80 rounded-xl shadow-md overflow-hidden nft-card-hover">
      <div className="h-48 bg-gradient-to-br from-oracle-orange/20 to-cosmic-connection/20 relative">
        {/* Loading state */}
        {imageLoading && (
      ...
      <div className="p-4">
        <h3 className="text-xl font-heading text-oracle-orange mb-2">{nftMetadata?.name || nft.name}</h3>
        ...
        <Link 
          href={`/nft/${nft.contractAddress}/${nft.id}`} 
