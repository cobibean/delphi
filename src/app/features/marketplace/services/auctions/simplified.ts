...
export async function checkIfNewWinningBid(
  ...
  try {
    ...
    // Convert string to BigInt for ThirdWeb functions
    const bidAmountBigInt = ethers.parseUnits(bidAmount, "ether");
...
export async function createAuctionSimplified(
  ...
  try {
    ...
    // Convert string values to BigInt for ThirdWeb functions
    const minimumBidAmount = params.minimumBidAmount 
      ? ethers.parseUnits(params.minimumBidAmount, "ether")
      : 0n;
      
    // Convert buyout amount to BigInt or undefined
    let buyoutBidAmount: bigint | undefined = undefined;
    if (params.buyoutAmount && params.buyoutAmount !== "") {
      buyoutBidAmount = ethers.parseUnits(params.buyoutAmount, "ether");
    }

    // Create auction transaction according to ThirdWeb docs
    const transaction = createAuctionThirdweb({
      ...
      minimumBidAmount: minimumBidAmount.toString() as any,
      buyoutBidAmount: buyoutBidAmount ? buyoutBidAmount.toString() as any : undefined,
...
export async function placeBidSimplified(
  ...
  try {
    ...
    try {
      ...
      // Convert bid amount to BigInt for ThirdWeb functions
      const bidAmountBigInt = ethers.parseUnits(params.bidAmount, "ether");
      ...
      const transaction = bidInAuction({
        contract: marketplaceContract,
        auctionId: BigInt(params.auctionId),
        bidAmount: bidAmountBigInt.toString() as any
      });
